...
namespace Memory::tui {
    ...
    void SetupScreen::startGame() {
        ...
        if (width * 7 > getInstance()->getSize().width || height * 7 + 2 > getInstance()->getSize().height) {
            Memory::game::Application::throwError("Board is too big for the screen(Max size height: " +
                                                  std::to_string((getInstance()->getSize().height - 2) / 7) +
                                                  ", width: " + std::to_string(getInstance()->getSize().width / 7) +
                                                  ")");
            return;
        }
