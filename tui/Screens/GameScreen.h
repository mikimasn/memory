...
namespace Memory::tui {
    ...
    private:
        ...
        int pending = -1;
    public:
        explicit GameScreen(const ElementParent &parent, Memory::game::Game *game) : Sheet(parent), game(game), player1({(int)game->getNames().first.size(), 1}, game->getNames().first), player2({(int)game->getNames().second.size(), 1}, game->getNames().second),
                                                                                     score({10, 1}, "0 - 0")  {
            instance = this;
            attachGame(game);
        };
        explicit GameScreen(const ElementParent &parent) : Sheet(parent), player1({10, 1}, "Player 1"), player2({10, 1}, "Player 2"),
                                                           score({10, 1}, "0 - 0") {
            instance = this;
        };
        void attachGame(Memory::game::Game *gameptr) {
            ...
            player2.updateParent(ElementParent{this, this->addChild(&player2, (int) game->getNames().first.size()+1, 0)});
            score.updateParent(ElementParent{this, this->addChild(&score, (int)(game->getNames().first.size()+game->getNames().second.size())+3, 0)});
            ...
            for(int i=0; i<height; i++){
                for(int j=0; j<width; j++){
                    ...
                    this->cards[index].updateParent(ElementParent{this, this->addChild(&this->cards[index], j*(cardWidth+cardhspacing), i*(cardHeight+cardvspacing)+1)});
                }
            }
        }
