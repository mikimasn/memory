...
namespace Memory::tui{
    ...
    private:
        ...
        void centerElements();
    public:
        explicit ResultScreen(const ElementParent &parent,const std::string &message): Sheet(parent), Title({10, 1}, "Game Ended"),
                                                                                       Desc({(int) message.size(),1},message),
                                                                                       closeButton({7,1},"[Close]",closeDialog),
