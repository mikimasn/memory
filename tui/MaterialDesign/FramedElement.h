...
namespace Memory::tui {
    ...
    public:
        ...
        FramedElement(Element *child, char c) : Element(
                (ElementSize) {child->getSize().width + 2, child->getSize().height + 2}), c(c) {
            registerChild(child);
        };

        FramedElement(Element *child, char c, const ElementParent &parent) : Element(
                (ElementSize) {child->getSize().width + 2, child->getSize().height + 2}, parent), c(c) {
            registerChild(child);
        };
